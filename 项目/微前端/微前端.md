...
- iframe集成：要求不高这个的话可用
    - 子项目形式：iframe嵌套
    - 缺点：每次都需要重新加载
- js集成：一个子项目对应一个script标签，script中把子项目挂载到windows上，需要的时候调用渲染
    - 子项目形式：独立的bundle
    - 优点：
        - 单独部署
        - 灵活性：何时渲染、传递参数
...
1. 容器通过路由来渲染来渲染子项目（js集成的方式）
    - 容器维护一个全局history对象（子项目共用容器的history），匹配对应的子项目
    - 请求manifest文件，获取子项目的资源路径
    - 构建script标签，获取子项目资源文件
    - 执行挂载在windows上的子项目的方法，渲染子项目
